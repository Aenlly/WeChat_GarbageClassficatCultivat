...
Page({
  ...
  submitForm(e) {
    ...
    if (text == '' || text == null) {
      _this.setData({
        errorMsg: this.data.rules.text
      })
      return
    } else if (textDesc == '' || textDesc == null) {
      _this.setData({
        errorMsg: this.data.rules.textDesc
      })
      return
    } else if (textTag == '' || textTag == null) {
      _this.setData({
        errorMsg: this.data.rules.textTag
      })
      return
  ...
  async chooseVideo() {
    ...
    if (_this.data.videoState == 0) {
      ...
      return
    } else if (_this.data.videoState == 1) {
      wx.showModal({
        title: '警告',
        content: '该操作会覆盖原视频！',
        success(res) {
          if (res.confirm) {
            _this.data.videoState = -1
